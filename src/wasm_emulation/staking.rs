...
use super::input::StakingStorage;
use super::query::staking::StakingQuerier;
...
pub trait Staking: Module<ExecT = StakingMsg, QueryT = AccessibleStakingQuery, SudoT = StakingSudo> {}

pub trait Distribution: Module<ExecT = DistributionMsg, QueryT = Empty, SudoT = Empty> {}

pub const STARGATE_ALL_STAKING_QUERY_URL: &str = "/local.staking.all";

#[derive(Debug)]
pub enum AccessibleStakingQuery{
    StakingQuery(StakingQuery),
    AllQuery()
}
...
impl Module for StakeKeeper {
    ...
    type QueryT = AccessibleStakingQuery;
    ...
    fn query(
        ...
        block: &BlockInfo,
        request: AccessibleStakingQuery,
    ) -> AnyResult<Binary> {
        let staking_storage = prefixed_read(storage, NAMESPACE_STAKING);
        match request {

        	AccessibleStakingQuery::AllQuery() =>{
        		// Return all the staking state for passing it down to the querier
        		// denom: String,
			    // validators: Vec<Validator>,
			    // delegations: Vec<FullDelegation>,

			    // We need to query the validators and all local delegations
			    let validators = self.get_validators(&staking_storage)?;
			    let delegations: Vec<FullDelegation> = validators.iter()
			    	.map(|v|{

			    	}).collect();

			    let querier = StakingQuerier::new(
			    ...
			    Ok(to_binary(&StakingStorage{
			    	storage: querier
			    })?)

        	},
        	AccessibleStakingQuery::StakingQuery(request) => {
        		match request{
...
// No need to query the distribution keeper, they don't have a query endpoint.
#[derive(Default)]
