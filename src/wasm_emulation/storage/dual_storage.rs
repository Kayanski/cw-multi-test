...
fn get_key_bigint(mut key1: Vec<u8>, mut key2: Vec<u8>) -> (BigInt, BigInt){
	...
	(BigInt::from_bytes_be(Sign::Plus, &key1), BigInt::from_bytes_be(Sign::Plus, &key2))
}
...
impl Storage for DualStorage{
    ...
    fn next(&mut self, iterator_id: u32) -> BackendResult<Option<Record>>{
    	...
    	let is_valid_distant_key = next_distant.is_some()
    		&&  (
    			...
    			(
	    			...
	    			gte(next_distant.unwrap().key.clone(), iterator.distant_iter.start.clone().unwrap())
    	...
    	if let Some((key, value)) = key_value{
    		(Ok(Some((key.clone(), value.clone()))), GasInfo::with_externally_used((key.len() + value.len()) as u64))
    	}else{
    		(Ok(None), GasInfo::with_externally_used(GAS_COST_LAST_ITERATION))
    	}
    }
