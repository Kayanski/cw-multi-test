...
pub fn main(){
    ...
    app.execute_contract(Addr::unchecked(sender), Addr::unchecked(market), &ExecuteMsg::DepositStable{}, &coins(10_000, currency)).unwrap();
    ...
    app.migrate_contract(Addr::unchecked(contract_info.admin.clone().unwrap()), Addr::unchecked(market), &MigrateMsg{ t: "t".to_string()}, code_id).unwrap();
    

    // The query count message should error with a specific storage error

    let err = app.wrap().query_wasm_smart::<GetCountResponse>(market, &QueryMsg::GetCount {  }).unwrap_err();

    if !err.to_string().contains("counter_contract::state::State not found"){
        panic!("Error {} should contain counter_contract::state::State not found", err);
    }

    // Now we migrate back and deposit again
    app.migrate_contract(Addr::unchecked(contract_info.admin.unwrap()), Addr::unchecked(market), &Empty{}, contract_info.code_id).unwrap();
    app.execute_contract(Addr::unchecked(sender), Addr::unchecked(market), &ExecuteMsg::DepositStable{}, &coins(10_000, currency)).unwrap();
